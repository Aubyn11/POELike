...
// 4x4 矩阵求逆（用于法线变换，重命名避免与 URP Core.hlsl 内置 Inverse 冲突）
float4x4 MonsterInverse(float4x4 m)
{
...
// instanceID 即 entityIdx，_BatchOffset 指向本批次在共享 buffer 中的起始位置
void setup()
{
#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
    float4x4 mat        = _MatrixBuffer[_BatchOffset + unity_InstanceID];
    unity_ObjectToWorld = mat;
    unity_WorldToObject = transpose(MonsterInverse(mat));
#endif
}
