...
- **`启动ExcelConvert.bat` 当前同时支持发布产物和源码仓库模式**：会优先使用内部 `ExcelExporter / excelConvert` 发布产物；若只有源码仓库且本机可用 `dotnet`，则会先自动发布到约定目录后再运行；若这两类内部工具都不可用，则自动走仓库内降级链：先刷新 `equipment.xlsx`，再把 `common/excel/xls/*.xlsx` 全量导出到 `Assets/Cfg/*.pb`**
