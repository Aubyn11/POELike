...
- 负责 **怪物死亡后地面掉落装备名称的绘制、背景底板与鼠标移入高亮**
...
- **`GameSceneManager` 当前会过滤 `Monster` 死亡并按概率创建 `ItemData`，随后发布 `GroundItemDroppedEvent`**
- **`GroundItemLabelRenderer` 会在怪物位置绘制掉落装备名称**

- **标签默认带深色背景，鼠标移入时会按名称实际宽高整块高亮背景，同时文字提亮为白色**
- **点击地面掉落名称时，当前会先调用背包容量检测；若背包放不下，则在屏幕中下方提示“背包放不下了”**
- **只有背包放得下时，当前掉落才会真正转换为 `BagItemData` 入包，并从地面标签列表移除**
- **多个几乎落在同一点的掉落标签会自动向上错开**
